...
## 리스트
- 데이터를 순차적으로 저장하는 자료구조를 말한다. 중복된 데이터의 저장을 막지 않는다.

### 리스트의 종류
1. 배열 기반 리스트: 인덱스를 기반으로 하여 동작한다.
2. 연결리스트: 동적 할당을 기반으로 하여 동작한다.
...
### :baloon: 임의의 위치에 원소를 추가하거나 제거하는 연산을 많이 하는 경우 연결리스트의 사용을 고려해보자.
